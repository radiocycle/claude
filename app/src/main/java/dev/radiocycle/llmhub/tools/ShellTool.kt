...
/**
 * Runs a shell command inside the workspace. Android ships a POSIX-ish shell (`sh` / toybox), so
 * commands like `ls`, `cat`, `grep`, `find` and pipelines work in the app's own sandbox. When the
 * device is rooted and root is enabled in settings, the command is run through `su`, so it can
...
class ShellTool(
    ...
    override val spec = ToolSpec(
        ...
        description = "Run a shell command in the workspace directory and return its combined " +
            "stdout and stderr plus the exit code. A POSIX shell is available. State does not " +
            "persist between calls, so chain steps with && or ; and cd within a single command.",
    ...
    /** Wraps a path in single quotes so spaces and metacharacters survive the shell. */
